...
| 6 | Self-update + anonymous update-check ping + Settings | FM-11, FM-13 | **shipped** (2026-09-21) — live check verified against the real repo (no releases yet → honest report); ping payload captured and asserted to the 4-field PRIVACY.md contract; opt-out via Settings toggle and env both proven. Releases flip from draft at the slice-8 release pass |
| 7 | Offline verification pass + branding pass (Geist, tokens) + single "There's more" link | FM-10, FM-17, FM-18 | **shipped** (2026-09-21) — fonts bundled locally and verified rendering offline; all data flows verified with outbound endpoints dead; vocabulary-ban audit clean (zero matches); exactly one outbound link, system-browser opened |
| 8 | Release engineering: v0.1.0 — Authenticode signing, notarized DMG, winget/scoop PRs, Docker (ghcr.io), screenshots → demo GIF | FM-1, FM-2, FM-14, FM-20 full pass | **pipeline complete** (2026-09-21) — macOS job, ghcr image, winget templates, runbook (RELEASE.md), real screenshots captured; validated end-to-end via throwaway tag → draft release. **Public v0.1.0 is blocked on credentials only you can create**: Windows Authenticode cert (Certum or Azure Trusted Signing) + Apple Developer account. Dockerfile + DMG packaging verified at the CI level |
